...
@app.get("/")
def root():
    return {"ok": True, "name": "Kelime Asistanı API", "wordCount": len(DICT_INDEX.word_set), "files": len(list(DATA_DIR.glob('*'))) if DATA_DIR.exists() else 0}
...
def solve(payload: SolveRequest):
    try:
        settings={
            "fast": {"limit":60, "seconds":2.0, "checks":90000, "all_starts":False, "extra":2},
            "deep": {"limit":180, "seconds":8.0, "checks":500000, "all_starts":False, "extra":8},
            "max": {"limit":500, "seconds":30.0, "checks":3000000, "all_starts":True, "extra":24},
        }[payload.mode]
        suggestions=generate_moves(board=payload.board, rack=payload.rack, index=DICT_INDEX, **settings)
